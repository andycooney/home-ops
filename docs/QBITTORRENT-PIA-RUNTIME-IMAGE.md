...
Loopback and established/related flows are allowed. Explicit non-WAN subnets and the configured inbound qBittorrent HTTP port are narrow exceptions. Before UID 1000's non-tunnel DROP, family-matched rules allow only ESTABLISHED TCP replies sourced from that service port to configured internal subnets; unrestricted ESTABLISHED,RELATED output remains after the UID block. During bootstrap only UID 0 gets DNS and HTTPS. Registration replaces that broad bootstrap access with TLS to one candidate IP/port. Tunnel startup replaces it with the exact UDP WireGuard endpoint. Root verification may use `tun0`; UID 1000 cannot initiate non-`tun0` traffic and receives `tun0` permission only in `HEALTHY`.
