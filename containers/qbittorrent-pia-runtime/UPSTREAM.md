...
The runtime fetches `https://serverlist.piaservers.net/vpninfo/servers/v6`, posts credentials as multipart fields to PIA's token endpoint, generates a new X25519 keypair, and calls `https://<validated-hostname>:1337/addKey` while dialing the selected IP directly. TLS SNI and hostname verification remain enabled against the vendored PIA CA. The registered session supplies the peer address, server key, server port, private `server_ip` gateway, and DNS servers. The private registered `server_ip` is the active PF API gateway on TCP port 19999; the public selected IP remains only the WireGuard endpoint.
